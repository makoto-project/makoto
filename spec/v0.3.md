# Makoto v0.3 Project Specification

**Status:** Unreleased implementation draft for review. The repository contains the v0.3 schema family, focused conformance fixtures, and reference implementation; no hosted publication or v0.3 release is claimed.
...
**Release refs:** `v0.3.0` is reserved for a future reviewed release. A candidate checksum inventory has a null tag and is not release evidence.

**Normative language:** “MUST”, “MUST NOT”, “SHOULD”, “SHOULD NOT”, and “MAY” are to be interpreted as described in RFC 2119 and RFC 8174.

## Changes from v0.2

Makoto v0.3 makes three protocol changes:

1. It adds the standard licence-claim profile. A signed statement can carry one SPDX licence expression and one evidence URL for each subject. A trust-policy rule can require the exact digest-pinned profile through `profileConstraints`. The claim identifies the signer that vouched for the declaration. It does not establish that the declaration is true or that the data is legally licensed.
2. It adds an optional `recordMerkle` commitment to each dataset-manifest entry, plus record-declaration and record-inclusion-proof schemas. Producers declare record boundaries as byte ranges or provide a record-digest list. Makoto does not parse Parquet, CSV, or another data format.
3. It adds Sigstore keyless signatures beside pinned Ed25519 signatures. A receiver policy pins exact OIDC issuer and subject values, optional exact GitHub Actions repository/workflow-ref/event claims, and the digest of a receiver-supplied Sigstore trusted root. Verification uses only the envelope, its embedded Sigstore bundle, and that separately supplied root.

All Makoto-owned identifiers for these shapes use the `/v0.3/` family. A verifier dispatches v0.2 and v0.3 separately and rejects a graph, bundle, handoff, policy, or profile that mixes the two families. The v0.2 schemas, identifiers, bytes, and decisions remain unchanged.

Index hierarchy, language metadata, sharding, scheduling, and AI-specific concepts remain extension or application concerns.

## v0.3 design decisions

The standard profile ID is `https://usemakoto.dev/profile/v0.3/license-claim-v1.schema.json`. Its signed statement extension is `https://usemakoto.dev/claim/v0.3/license`. Claims are sorted by `subjectName`, cover the statement's subjects exactly once, contain a syntactically valid SPDX 2.x expression, and name an absolute HTTPS evidence URL without a fragment. The verifier owns and checksum-pins the profile bytes. This makes the policy requirement portable without treating a producer catalogue as a trust root.

Record commitments use `rfc6962-sha256-record-digest-v1`. For a record digest `D`, the leaf is `SHA-256(0x00 || D)`. An internal node is `SHA-256(0x01 || left || right)`. Trees use the RFC 6962 shape: an unpaired node advances unchanged to the next level. This construction is established, domain-separates leaves from nodes, supports logarithmic inclusion proofs, and gives byte-range and record-hash declarations the same commitment format.

A byte-range declaration hashes the exact bytes at each producer-declared `(offset, length)`. A record-hash declaration supplies those SHA-256 record digests directly. The signed dataset manifest stores only the algorithm, record count, and root. A proof binds the verified manifest statement and subject, dataset entry and digest, record index and digest, optional byte range, and audit path. Verification first requires an allowed v0.3 bundle, then checks the signed entry commitment and proof. Without supplied record bytes, the result proves inclusion of the declared record digest. With supplied bytes, it also checks that their SHA-256 equals that digest.

v0.3 does not add a VSA verified property for record inclusion. A record proof is scoped to one operation and may be checked after the bundle report was produced; adding it to the bundle-wide VSA would overstate what the VSA established.

Sigstore keyless signatures use a Fulcio short-lived signing certificate bound to an OIDC identity and a Rekor transparency-log entry. The envelope signature entry carries a complete Sigstore bundle. Its `keyid` is the SHA-256 of the RFC 8785 JSON identity constraint, not an ephemeral certificate-key digest. The verifier checks the Sigstore bundle against the exact outer DSSE payload and signature before applying receiver authorization.

The Python reference uses the maintained `sigstore` package's public signing, trusted-root, and DSSE verification APIs. Those APIs implement the Fulcio certificate, signed-certificate-timestamp, Rekor inclusion, checkpoint, and trusted-time checks; duplicating that security-sensitive logic locally would add a second verifier without adding a Makoto-specific capability.

The producer may contact Fulcio, Rekor, a timestamp authority, TUF, and an OIDC provider while signing. Verification MUST NOT use the network. The receiver supplies a Sigstore trusted-root JSON file separately and pins its exact SHA-256 in `sigstoreTrustRoot`; producer bundle contents can never add or replace trust anchors. Pinned Ed25519 keys remain supported, and one threshold may count distinct passing Ed25519 key IDs and keyless identity IDs.

GitHub Actions identity constraints contain exact `issuer`, `subject`, `repository`, `workflowRef`, and `event` strings. They do not accept regular expressions, repository prefixes, branch globs, or producer-selected claim values. The reference workflow uses GitHub's OIDC token only for signing and grants `id-token: write` with read-only repository contents.

No v0.3 tag or GitHub release existed when this change was made. Because v0.3 was still an unreleased implementation draft, its accepted envelope and trust-policy bytes were extended in the existing `/v0.3/` family before publication. Once a v0.3 release exists, the protocol-identifier rule in Section 17 requires a new complete identifier family for another validation or wire-semantics change.
...
The checked-in end-to-end demonstration remains a v0.2 compatibility proof. v0.3 adds focused licence-profile, record-proof, and offline keyless-verification fixtures and tests; it does not relabel the v0.2 demo or claim a hosted v0.3 walkthrough.

This document carries the complete v0.2 protocol contract forward under a separate v0.3 identifier family and states the three v0.3 additions above. A release claim requires the reviewed prose, immutable schemas, verification-report schema, versioned `diagnostic-map.json`, applicable conformance vectors, and tagged checksum manifest. The current draft and candidate checksum inventory are not interoperability, hosting, or release evidence.

## 2. Why v0.3 is narrow
...
Makoto v0.2 established the signed statement, authorization, graph, profile, handoff, and verification-report model described below. v0.3 does not redesign that model. It creates a separate identifier family because record proofs, the dataset-manifest field, and keyless envelope/policy entries change accepted shapes and decisions, while the licence profile supplies a new verifier-owned policy target. The v0.2 bytes and decisions remain immutable.

### 2.1 Verified current-state audit

This subsection is inherited, non-normative evidence from the v0.2 design audit. Its repository counts describe that historical baseline and are not v0.3 implementation or conformance claims.
...
- Define one coherent v0.3 representation for origins, transformations, artifact bindings, profiles, signatures, graph edges, and handoffs.
...
- Ship a reference CLI and library that can create, sign, bundle, and verify v0.3 provenance.
- Make the complete demonstration runnable from a clean clone with one documented command sequence.
- Host immutable, CORS-enabled v0.3 schemas at `usemakoto.dev`.
- Make the website’s examples, downloads, CLI behavior, and hosted schemas agree byte-for-byte.
- Preserve v0.1 URLs and clearly label v0.1 as historical and incompatible with v0.3.
...
- Global key discovery, public revocation infrastructure, or certificate federation beyond the receiver-pinned Sigstore trust root.
...
- Validation of arbitrary CSV, Parquet, images, audio, or proprietary binary semantics. v0.3 content validation is normative for JSON artifacts; other formats require future adapters.
- Package publication to PyPI or npm. Local installation from the repository is sufficient for v0.3 acceptance.
...
| Track | A family of cumulative, producer-adoptable controls for one part of data production. v0.3 defines Origin and Transform tracks. |
...
| DBOM | Human shorthand for the Makoto provenance graph and handoff evidence for data. In v0.3 it is not a separate mutable lineage document. |
...
The standard licence-claim profile does not change this boundary. It records the exact licence expression and evidence URL a signer vouched for, and lets a receiver require that signed claim. Makoto does not decide whether the expression is correct, the evidence is sufficient, the signer has authority, or the data is legally usable.
...
- Statements MUST be carried in DSSE envelopes for a handoff to qualify as authenticated Makoto v0.3 provenance.
- A statement envelope's DSSE `payloadType` MUST be `application/vnd.in-toto+json`; a handoff envelope's type MUST be `application/vnd.makoto.handoff.v0.3+json`. `envelope.schema.json` requires `payloadType` to be an ASCII string of at most 255 bytes with exactly one `/`. Its type and subtype are each 1 through 127 characters, begin with a lowercase ASCII letter or digit, and otherwise contain only lowercase ASCII letters, digits, `!`, `#`, `$`, `&`, `^`, `_`, `.`, `+`, or `-`; parameters, whitespace, and every other byte are forbidden. The schema deliberately does not enumerate the two supported values. A string that fails this lexical grammar is `E_ENVELOPE_MALFORMED`; a lexically valid string that is not the exact value required at that index position is `E_PAYLOAD_TYPE`. After canonical base64 decoding and strict JSON parsing, the verifier semantically dispatches the two supported values to `statement.schema.json` or `handoff.schema.json`; standard JSON Schema content keywords do not perform decoding or dispatch.
- The DSSE signature MUST cover the DSSE pre-authentication encoding of `payloadType` and the exact decoded `payload` bytes.
- v0.3 uses the DSSE v1 wire protocol. PAE is the byte concatenation `"DSSEv1" SP decimal_byte_length(payloadType) SP payloadType SP decimal_byte_length(payload) SP payload`, where `SP` is byte `0x20`, lengths are base-10 ASCII with no leading zeros except that the one-digit spelling `0` is valid for zero length, `payloadType` is UTF-8, and `payload` is the exact decoded byte sequence.
- Envelope `payload` and signature `sig` values MUST use RFC 4648 standard base64 with required padding and no whitespace. After decoding, re-encoding with canonical RFC 4648 rules MUST reproduce the exact input string; nonzero unused pad bits and every alternate spelling are rejected. An Ed25519 `sig` decodes to exactly 64 raw signature bytes. A Sigstore entry's `sig` MUST exactly equal the one signature in its embedded Sigstore DSSE envelope.
- Every DSSE envelope in v0.3 MUST contain at least one signature entry. An empty `signatures` array fails transport validation with `E_ENVELOPE_MALFORMED`, not the generic `E_CORE_SCHEMA`.
...
Makoto's in-toto/DSSE interoperability claim is deliberately limited to the outer in-toto Statement v1 and DSSE v1 wire formats, PAE, payload parsing, and standard cryptographic primitives. It does not promise that arbitrary foreign envelopes satisfy the narrower Makoto profile, key, graph, or policy contract. A conforming v0.3 verifier may intentionally reject foreign key-ID conventions, unsupported co-signature shapes, non-Ed25519 policy keys, multiple digest algorithms on subjects, extension predicate semantics, or any otherwise valid in-toto/DSSE feature outside the closed v0.3 subset. Documentation MUST say “parseable outer format,” not imply drop-in policy or semantic compatibility with every in-toto implementation.

### 10.2 Digests

- v0.3 MUST support SHA-256 and MUST use lowercase hexadecimal encoding in JSON digest objects.
...
The v0.3 digest object contains exactly one member named `sha256`; all other members are rejected with closed-object schema validation. The enclosing field shape can be versioned later to add algorithms without ambiguity.

Adding or replacing a required digest algorithm changes protocol validation semantics and therefore requires a new versioned schema and predicate identifier. A future version may permit multiple algorithms during a transition, but a v0.3 verifier MUST NOT negotiate or silently substitute another algorithm.

### 10.3 Digital signatures

- DSSE remains algorithm-neutral. The v0.3 trust-policy schema and reference implementation accept pinned Ed25519 verification keys and Sigstore keyless identities. Pinned-key verification MUST reject noncanonical `S >= L`, noncanonical point encodings, small-order public keys, and small-order `R` values, following the pinned strict vectors rather than permissive library defaults. A non-Ed25519 entry in `keys` is invalid v0.3 consumer configuration and exits 2 before evidence evaluation.
...
- Every v0.3 DSSE signature entry MUST contain a `keyid` with the form `sha256:<64 lowercase hex>`. For Ed25519 it hashes canonical DER SubjectPublicKeyInfo bytes. The only accepted SPKI encoding is the 44-byte RFC 8410 DER sequence `302a300506032b6570032100 || 32-byte-public-key`: OID `1.3.101.112`, absent algorithm parameters, a zero-unused-bit BIT STRING, no trailing DER, and no alternate encoding. Policy `publicKey` is canonical RFC 4648 padded base64 of exactly those 44 DER bytes. For keyless signing, `keyid` hashes the RFC 8785 JSON bytes of the closed identity constraint. The verifier MUST recompute the applicable identifier; a policy map key that differs is invalid consumer configuration with exit 2. Evidence lookup uses exact `keyid` matching and MUST NOT trial-verify against unrelated keys or identities.
- A Sigstore signature entry has exactly `keyid`, `sig`, and `sigstoreBundle`. The bundle MUST contain exactly one DSSE signature and the exact outer `payloadType`, decoded payload bytes, and `sig`. The verifier MUST validate the Fulcio chain and certificate profile, exact identity claims, signed certificate timestamp, Rekor inclusion proof and checkpoint, log-entry consistency, and a verified signing time within the short-lived certificate validity period. An expired certificate is accepted only when the bundle provides a valid trusted time inside its validity interval.
- `sigstoreTrustRoot` is a receiver policy resource descriptor. Its SHA-256 covers the exact separately supplied trusted-root JSON bytes. A policy with `keylessIdentities` and no root descriptor is invalid. A keyless signature cannot pass when the receiver omits the separately supplied root, supplies bytes with another digest, or supplies a root that cannot validate the bundle. Verification constructs the Sigstore verifier directly from those pinned bytes and MUST NOT refresh TUF or call Fulcio, Rekor, a timestamp authority, or an OIDC provider.
- Each `keylessIdentities` value names exact OIDC `issuer` and `subject` strings. A GitHub Actions identity also names exact `repository`, `workflowRef`, and `event` certificate-extension strings. Rules and the handoff policy use `authorizedIdentityIds` beside `authorizedKeyIds`; `minimumSignatures` counts the union of distinct passing configured signer IDs. Both fields may appear in one rule.
...
- A `keyid` identifies a configured verification key or exact keyless identity constraint; it is not an authorization decision.
...
- A structurally malformed signature entry, missing/invalid `keyid`, noncanonical base64, an invalid algorithm-specific signature length, or duplicate `keyid` fails at Step 2 with `E_ENVELOPE_MALFORMED`; no signature record is emitted for an entry without a usable signer ID. A structurally valid signature claiming a configured Ed25519 key or keyless identity that fails its cryptographic verification fails at Step 5 with `E_SIGNATURE_INVALID`. A structurally valid signature from an unknown signer cannot be named by policy because every authorized signer must be configured; it does not count toward a threshold and produces `W_SIGNATURE_UNKNOWN`, not an independent denial. Envelopes are never partially repaired.
...
- Everywhere in v0.3, a `canonical lowercase UUID` means exactly `[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}`. The nil UUID is allowed and version/variant bits are not constrained unless a field explicitly adds a rule.
- `event.id` is either an absolute URI or a canonical lowercase UUID string matching exactly `[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}`. The nil UUID is allowed; version and variant bits are not constrained in v0.3. For every v0.3 URI-or-UUID field, including `event.id` and `bundleId`, any value whose case-insensitive prefix is `urn:uuid:` MUST use the exact lowercase prefix `urn:uuid:` followed by that exact lowercase UUID grammar; an uppercase or mixed-case prefix/value is invalid and cannot bypass UUID lexical rules through the generic absolute-URI branch. Graph uniqueness compares complete event-ID JSON strings exactly: a bare UUID and `urn:uuid:` plus the same UUID are distinct identifiers. UUID uniqueness, not UUID version semantics, is enforced.
- v0.3 media types are lowercase RFC 6838 type/subtype tokens without parameters.
...
```json
{
  ...
  "predicateType": "https://usemakoto.dev/predicate/v0.3/transform",
  "predicate": {}
}
```
...
- `predicateType` MAY name a Makoto core or extension predicate, but the v0.3 reference bundle verifier accepts only the core origin and transformation types; consumer policy cannot override missing graph semantics.
- The core statement schema MUST allow extension predicate types without claiming to understand them.

The v0.3 reference verifier has graph semantics only for the core origin and transformation predicates. Another predicate type MAY be parsed and inspected out of band, but every v0.3 bundle verifier MUST reject it when it appears in the manifest statement set or reachable graph with `E_PREDICATE_SEMANTICS_UNSUPPORTED`. Semantic adapters are outside the v0.3 protocol and cannot change a v0.3 bundle decision; a future adapter contract must pin its identity, bytes, authorization, predecessor semantics, and report fields under a new protocol version. An implementation MUST NOT label an extension predicate as authenticated auxiliary evidence, treat it as an origin, or skip it during root discovery.
...
|---|---|---:|---|
| `schemaVersion` | string constant `0.3` | yes | Makoto predicate version. |
...
Predicate type: `https://usemakoto.dev/predicate/v0.3/origin`
...
Predicate type: `https://usemakoto.dev/predicate/v0.3/transform`
...
#### Standard licence-claim profile

The verifier-owned standard profile has ID `https://usemakoto.dev/profile/v0.3/license-claim-v1.schema.json`, SHA-256 `4b10b1e2500edaa095c7771d74f95271acc31b8313a3954cbdbe6a4e4b055a80`, byte length 2115, target `statement`, `critical: true`, and an empty resource closure. Its exact bytes are `src/makoto/standard-profiles/v0.3/license-claim-v1.schema.json`. Startup verifies the byte length, digest, and root `$id` before accepting the profile.

The profile requires `predicate.extensions["https://usemakoto.dev/claim/v0.3/license"].claims`. Each claim has exactly `subjectName`, `spdxExpression`, and `evidenceUrl`. Claims MUST be sorted by `subjectName` UTF-8 bytes, MUST NOT repeat a subject, and MUST cover every statement subject exactly once. `spdxExpression` follows the SPDX 2.x expression grammar, including `AND`, `OR`, `WITH`, parentheses, SPDX identifiers, `LicenseRef-` identifiers, and `DocumentRef-...:LicenseRef-...` identifiers. The parser validates syntax without consulting a mutable licence-list release. `evidenceUrl` MUST be an absolute HTTPS URL with an authority and no fragment.

A receiver requires this claim by adding the profile's exact `id`, `digest`, `closureDigest`, and `target: "statement"` to a rule's `profileConstraints`. An absent profile cannot match the rule. An attached profile with missing subject coverage, malformed SPDX syntax, or an invalid evidence URL fails profile validation with `E_PROFILE_INVALID`.
...
`contentEncoding`, `contentMediaType`, and `contentSchema` are annotation-only in v0.3 and never change instance validity. `contentSchema` is still traversed at load time as a schema-bearing position so its references must be in the pinned closure and its schema syntax is checked, but it is never entered during instance evaluation or charged as an instance-evaluation branch.
...
Profile-owned root and transitive resources use the Makoto profile dialect. Immutable core-catalog schemas are the sole exemption: each keeps its declared generic Draft 2020-12 dialect and is evaluated under that dialect when referenced from `allOf`. Makoto v0.3 core schemas MUST NOT use standard `pattern` or `patternProperties`; protocol lexical checks remain semantic verifier rules so no host-regex dialect can affect core validity. A profile cannot cause a core resource to inherit the profile dialect or shadow its `$schema`.

`critical` is an explicit boolean chosen by the producer for that profile reference. It means the producer considers successful resolution and validation necessary to interpret the evidence. It does not let the producer weaken consumer requirements: a trust policy MAY require an artifact profile by ID, digest, subject name, and media type even if the producer omits it or marks it noncritical. Consumer `requiredProfiles` are restricted to artifact targets in v0.3 and bind to final handoff artifacts as defined in Section 14; an intermediate same-named subject cannot satisfy them. An ordinary value under `extensions` has no criticality flag; requirements that must fail closed belong in a profile.
...
The optional v0.3 dataset-manifest media type is `application/vnd.makoto.dataset-manifest.v0.3+json`:
...
- A dataset-manifest subject MUST carry a critical artifact-target profile reference whose ID, root digest, recomputed closure digest, and target subject exactly match the immutable core-catalog schema entry for `https://usemakoto.dev/schema/v0.3/dataset-manifest.schema.json`; the signed profile reference's media type MUST independently equal the prose-fixed `application/vnd.makoto.dataset-manifest.v0.3+json`. The catalog entry has no media-type member, and ID alone is insufficient.
...
{
  "recordMerkle": {
    "algorithm": "rfc6962-sha256-record-digest-v1",
    "recordCount": 1000000,
    "root": { "sha256": "..." }
  }
}
...
`recordCount` is from 1 through `9223372036854775807`. `root` is the Merkle root defined in the v0.3 design decisions. This field commits to producer-declared records inside the exact entry bytes. It does not assert that the boundaries match rows in Parquet, CSV, or another logical format.

The `record-declaration` schema defines two producer inputs. `kind: "byteRanges"` carries a nonempty, sorted, non-overlapping array of `(offset, length)` pairs. Each range MUST fit inside the exact entry bytes; gaps are allowed. Its record digest is SHA-256 over the exact selected bytes. `kind: "recordHashes"` carries a nonempty ordered array of SHA-256 digest objects. Makoto treats that order as the producer's record order and does not infer or parse boundaries.

The `record-inclusion-proof` schema contains protocol version, algorithm, manifest statement digest, manifest subject name, entry name and digest, record index and count, record digest, an optional byte range, and the RFC 6962 audit path. `recordIndex` is zero-based and less than `recordCount`. Proof hashes are ordered from the leaf level toward the root. The audit path has at most 63 hashes.

`makoto record prove` first verifies the named bundle under the supplied consumer policy and requires an `allow` report for protocol 0.3. It then requires the exact signed dataset-manifest subject, matches the entry digest and `recordMerkle`, checks the declaration root and count, and emits the proof. `makoto record verify` repeats bundle verification, matches the proof identity to that verified entry, and checks the audit path. If exact record bytes are supplied, it also checks their SHA-256. If exact entry bytes and a proof byte range are supplied, it checks the entry digest, extracts that range without parsing the file format, and checks the record digest.

Root construction is O(n) SHA-256 operations and O(n) transient memory in the reference implementation. A proof carries at most `ceil(log2(n))` hashes and verification is O(log n). The conformance performance test builds and proves one member of a 1,000,000-record tree and requires root plus audit-path generation within 15 seconds on CI; the threshold is a regression bound, not a protocol guarantee.
...
The handoff DSSE `payloadType` is `application/vnd.makoto.handoff.v0.3+json`.
...
```json
{
  ...
  "rules": [
    ...
    },
    {
      ...
      "sourceUris": ["urn:makoto:demo:v0.3:source:customers-raw"],
...
A valid but unauthorized signature MUST fail policy. A missing rule MUST fail closed. The required top-level policy fields are `version`, `keys`, `rules`, `handoff`, `requiredProfiles`, and `limits`; `requiredProfiles` is required but may be an empty array. `keylessIdentities` and `sigstoreTrustRoot` are optional together: a policy with identities requires the root descriptor. Top-level `rules` MUST be duplicate-free by `id` and sorted by `id` using UTF-8 bytes. Top-level `requiredProfiles` MUST be duplicate-free by the complete tuple and sorted by `(id, digest.sha256, closureDigest.sha256, target, subjectName, mediaType, scope)` using the Section 13.2 string-order rules; distinct digest-pinned versions with the same ID/media type may be conjunctively required. Each rule's `profileConstraints` array, when present, is likewise duplicate-free and sorted by `(id, target, digest-or-empty, closureDigest-or-empty)`; paired digest omission sorts before a supplied digest pair. The handoff signer arrays and every other scalar policy array are duplicate-free and sorted by UTF-8 bytes. The `limits` object requires every member shown in the example and defines no defaults; a consumer must choose explicit values within bootstrap ceilings. Human labels are display metadata only.
...
6. v0.3 has no explicit deny-rule or first-match precedence. A future deny language requires a new trust-policy version.
...
Rule `A` subsumes rule `B` only when: `A.predicateTypes` is a superset of `B`'s; the union of `A.authorizedKeyIds` and `A.authorizedIdentityIds` is a superset of the corresponding union for `B`; `A.minimumSignatures <= B.minimumSignatures`; each optional `sourceKinds`, `sourceUris`, or `operationTypes` field is absent in `A` or, when present in `A`, is present in `B` and is a superset of `B`'s corresponding set; and every profile constraint in `A` matches at least one constraint in `B` under this per-member relation: `id` and `target` are exact, omitted root/closure digests in `A` match either paired digest state in `B`, and present digests in `A` match only the same exact root and closure digests in `B`. Thus omission is the universal set for overlap lint: omitted `A` subsumes supplied or omitted `B`, while supplied `A` never subsumes omitted `B`.

That subsumption is **strict** exactly when all preceding non-strict relations hold and at least one of these syntactic conditions holds: `A.predicateTypes` or its authorized-signer union is a proper superset; `A.minimumSignatures` is lower; one optional selector field is absent in `A` but present in `B`; one optional selector field present in both is a proper superset in `A`; `B` has a profile constraint not used to match any constraint in `A`; or a matched constraint omits the digest pair in `A` and supplies it in `B`. No semantic URI relationship, schema implication, signer equivalence, or data-dependent satisfiability is inferred. Only then emit `W_POLICY_RULE_OVERLAP` for ordered pair `(A.id, B.id)`.
...
Public-key revocation services and OIDC identity federation are out of scope for the v0.3 reference implementation. A local policy MAY remove a compromised Ed25519 key or keyless identity. It may also constrain an Ed25519 key with `validFrom` and `validUntil`. Those bounds are evaluated against the verifier's current time or an explicit consumer-supplied evaluation time, never against the signer-asserted `event.occurredAt`. Backdating an event cannot restore authorization. Sigstore certificate-chain and transparency-log validation instead use the bundle's trusted signing-time evidence and the receiver-pinned trust root. Historical authorization still requires the applicable archived trust-policy snapshot or continued authorization of the signer ID.
...
In Step 8, the media-hint candidate is exactly a signed handoff-manifest final-artifact entry whose optional `artifacts[].mediaType` is `application/vnd.makoto.dataset-manifest.v0.3+json`. The closed unsigned `bundle.json.artifacts[]` mapping has no `mediaType` member and never supplies this candidate signal. Phase 0 includes a media-hint-only candidate with no mandatory signed profile and expects Step 8 `E_DATASET_MANIFEST_INVALID`.
...
These command and subcommand names, required flags, exit codes, and JSON report fields are normative for the v0.3 reference CLI. Implementation may add backward-compatible optional flags. Any breaking CLI change requires revision of this specification before code changes and an atomic update to tests, demo scripts, and website copy.
...
The v0.3 error-code enum is closed to:
...
The v0.3 warning-code enum is closed to `W_PROFILE_INDETERMINATE`, `W_PROFILE_RESOURCE_LIMIT`, `W_ARTIFACT_VALIDATION_LIMIT`, `W_HISTORICAL_ARTIFACT_NOT_CHECKED`, `W_SIGNATURE_UNKNOWN`, `W_FRESHNESS_NOT_CHECKED`, `W_ARTIFACT_UNPROFILED`, and `W_POLICY_RULE_OVERLAP`. Any new code requires a new `reportVersion`; implementation-defined codes are forbidden in a `0.3` report.
...
```text
https://usemakoto.dev/schema/v0.3/envelope.schema.json
https://usemakoto.dev/schema/v0.3/statement.schema.json
https://usemakoto.dev/schema/v0.3/origin.schema.json
https://usemakoto.dev/schema/v0.3/transform.schema.json
https://usemakoto.dev/schema/v0.3/profile-reference.schema.json
https://usemakoto.dev/schema/v0.3/profile-dialect.schema.json
https://usemakoto.dev/schema/v0.3/catalog.schema.json
https://usemakoto.dev/schema/v0.3/dataset-manifest.schema.json
https://usemakoto.dev/schema/v0.3/record-declaration.schema.json
https://usemakoto.dev/schema/v0.3/record-inclusion-proof.schema.json
https://usemakoto.dev/schema/v0.3/handoff.schema.json
https://usemakoto.dev/schema/v0.3/bundle.schema.json
https://usemakoto.dev/schema/v0.3/trust-policy.schema.json
https://usemakoto.dev/schema/v0.3/verification-report.schema.json
https://usemakoto.dev/schema/v0.3/catalog.json
```
...
| `dataset-manifest` | object | Sorted finite partition entries, digests, and optional record Merkle commitments. |
| `record-declaration` | object | Producer-declared byte ranges or ordered record digests used to construct a commitment. |
| `record-inclusion-proof` | object | One record digest's audit path and binding to a verified dataset entry. |
...
- The v0.1 and v0.3 pages MUST state that the formats are incompatible.
...
The canonical core release checksum manifest is `release/v0.3/checksums.json`, validated by the checked-in closed tooling schema `release/checksums.schema.json`. It has exactly `{"files":[{"digest":{"sha256":<64-lowercase-hex>},"path":<repository-relative-path>}...],"tag":"v0.3.0","version":"1"}`. `files` is nonempty, duplicate-free by path, and sorted by path UTF-8 bytes. Paths use NFC `/`-separated repository-relative logical names under the Section 13.3 grammar. The file is strict UTF-8 JSON without BOM or duplicate keys and is serialized as RFC 8785 JCS plus one LF.

Checksum-manifest `version: "1"` versions this tooling inventory shape, not the Makoto wire protocol; it is intentionally distinct from protocol `0.3`. A future checksum-shape revision increments this integer-string contract without rewriting released protocol identifiers or bytes.

The checksum inclusion set is exact: every regular file under `examples/go/`, `schemas/v0.2/`, `schemas/v0.3/`, `testdata/v0.2/`, `testdata/v0.3/`, `demos/v0.2-end-to-end/`, `docs/`, `scripts/`, `src/makoto/`, and `tests/`; exact files `spec/v0.2.md`, `spec/v0.3.md`, `release/checksums.schema.json`, `pyproject.toml`, `uv.lock`, `LICENSE`, and `README.md`; and no other path. Including v0.2 in the v0.3 release inventory records the compatibility implementation and immutable prior contract; it does not revise the v0.2 schema family. Ignored `.work/`, tool caches, generated benchmark results not checked into `testdata`, the checksum manifest itself, and Git metadata are excluded. Each entry digest is SHA-256 over the exact Git blob/worktree bytes at the tagged commit, including any final LF, with no JCS reserialization, newline conversion, or archive transformation. Release generation runs from a clean checkout, derives the complete inclusion set, rejects missing/extra entries, and verifies every digest before tag creation and again from the peeled tag afterward. It additionally verifies that the eight JSON Schema registry resources and the standard licence-profile resource equal the IDs, lengths, and SHA-256 values in Section 12.1 and that both internal worker-result schema digests equal the constants asserted by `scripts/check.sh`.

`src/makoto/unicode/15.0.0/` contains exact vendored `UnicodeData.txt`, `CaseFolding.txt`, `CompositionExclusions.txt`, and `DerivedNormalizationProps.txt` inputs plus a closed sorted `unicode-catalog.json` that records each filename, Unicode version `15.0.0`, exact byte length, and SHA-256. NFC normalization and full case folding use only tables deterministically generated from those bytes; startup or `scripts/check.sh` rejects a catalog/input/generated-table mismatch. Phase 0 records upstream retrieval evidence and cross-checks normalization/case-fold vectors with the vendored version. The checksum inventory covers the portable v0.3 implementation, protocol schemas, documentation, tooling, tests, and conformance inputs named above; repository administration such as Git metadata and CI-provider configuration remains authenticated by the reviewed tag rather than duplicated in the manifest.

The checksum manifest does not self-authenticate; the independently pinned repository and reviewed Git tag target authenticate it. The release operator MUST verify that the tag names the reviewed commit, and the release archive MUST reproduce the same included bytes. Every `/schema/v0.3/` entry in website `schema/core-release.json` MUST map to and equal the corresponding `schemas/v0.3/` digest in this core checksum manifest. `scripts/release-check.sh` validates the closed schema, ordering, inclusion set, exact preimages, tag/commit relationship when a tag is present, and website-sync compatibility. The routine repository gate `scripts/check.sh` requires the same exact inclusion-set and preimage equality only for a tagged manifest. For the null-tag candidate it validates the closed schema, ordering, canonical serialization, and that the complete inclusion set regenerates, because a candidate is not release evidence and routine dependency updates change included bytes such as `uv.lock` without rewriting it. Changing the checksum shape or inclusion set requires a tooling-contract revision; changing any included protocol byte after release requires the protocol version-family rules.

## 18. Inherited v0.2 end-to-end demonstration

This section records the existing v0.2 acceptance demonstration. It is retained because the reference implementation continues to verify v0.2, but it is not a v0.3 conformance claim. The v0.3 work in this draft adds focused fixtures under `testdata/v0.3/` for licence claims, record commitments, and record inclusion proofs. It does not create, host, or relabel a v0.3 version of this demonstration.
...
## 19. Future v0.3 website requirements

No v0.3 website publication is part of this implementation draft. The requirements in this section are release gates for later publication, not claims about current deployed state.

The core v0.3 webpage must be the canonical narrative counterpart to the runnable repository demo. It must include:
...
```text
schemas/v0.3/                     canonical JSON Schemas and catalog
spec/v0.3.md                      normative protocol text
...
examples/go/                     schema-first Go integration examples and tests
tests/                            schema, crypto, graph, policy, and CLI tests
testdata/v0.3/                    valid and invalid conformance fixtures
testdata/v0.3/diagnostic-map.json stable code/step/context/continuation contract
...
release/v0.3/checksums.json       exact tagged release byte inventory
LICENSE                           authoritative license
README.md                         accurate v0.3 quick start and status
```

The core repository MUST publish a tagged v0.3 release artifact containing schemas, conformance fixtures, and checksums. It MUST NOT publish to a language package registry as part of this milestone.

### 20.2 Website repository

```text
schema/v0.3/                      exact release schema and catalog copies
schema/core-release.schema.json  closed release-index validation schema
spec/v0.3/                        rendered protocol documentation
...
- v0.3 is not wire-compatible with v0.1.
- v0.1 schema URLs and documentation remain available and immutable.
- v0.1 objects MUST never be accepted as v0.3 through permissive coercion.
- The v0.3 release ships a non-normative field-mapping and guarantee-loss guide, not an automated migration command or a legacy-acceptance mode. Conversion ordering, inventory formats, marker profiles, and opt-in policy require a separate versioned migration specification.
- A v0.1 document MAY be preserved as an ordinary exact-byte input artifact to a newly signed v0.3 origin or transformation. That v0.3 signature attests only to the new observation or conversion event and the digest of the presented legacy bytes; it MUST NOT be reported as authentication of the historical v0.1 lineage or signer.
- The v0.3 reference verifier has no `legacyUnverified` marker, inventory allowlist, `authenticated-history` shortcut, or policy exception. Any future converter that adds those concepts needs new schemas, report fields, error codes, and conformance fixtures before its output can participate in an `allow` decision.
...
- Valid signatures from unauthorized signers.
...
- No network retrieval in the v0.3 reference verifier.
- No arbitrary code execution from schema formats, extension fields, or bundles.
- Strict failure for malformed or invalid signatures claiming configured signers is a deliberate integrity posture; bundle corruption can already cause denial, and implementations MUST NOT silently repair ambiguous envelopes.
...
- Keep every hosted core schema below 250 KiB and the combined v0.3 schema set below 2 MiB.
...
- Published DSSE and Sigstore bundle interoperability vectors.
- Ed25519 positive, corrupt signature, wrong key, wrong payload type, malformed base64, and multiple-signature cases; Sigstore positive offline verification, wrong identity, wrong issuer, invalid trusted time, tampered log entry, and missing receiver trust root.
...
- Signature thresholds count distinct authorized signer IDs, duplicate IDs fail, unknown well-formed co-signatures warn without counting, and malformed or invalid configured-signer signatures fail.
...
- Record the exact candidate commit SHA, run `scripts/release-check.sh` from a clean checkout of that SHA, verify the checkout remains unchanged, and only then create immutable tag `v0.3.0` pointing to that same SHA.
- Run `uv run scripts/sync_core_release.py --tag v0.3.0` to replace the candidate pin with the verified tag/commit, review that only release-reference metadata changed, run the same site check locally, merge the website change to `main`, and let `.github/workflows/deploy.yml` publish `main` to `gh-pages` only after the check job succeeds.
- Run a post-deploy probe that compares every hosted schema byte digest with `schema/core-release.json`, checks status/content type/CORS, loads the v0.3 walkthrough, and follows its core tag link.
...
Rollback is documentation-safe: v0.1 remains immutable; v0.3 draft URLs are not promoted as stable until all release gates pass. Once v0.3 versioned schemas are declared released, their bytes cannot be rolled back or edited; corrections require v0.3.1 or v0.3 identifiers according to compatibility impact.
...
v0.3 is successful when all of the following are true:
...
- 100% of valid v0.3 statements and manifests use genuine digital signatures; zero bare hashes are labeled signatures.
...
Adoption counts, foundation acceptance, external integrations, and marketing reach are not v0.3 release criteria.
...
| JSON Schema is oversold as universal data validation | Binary/columnar claims are false | Normatively support JSON artifacts only in v0.3 and require future adapters. |
| Private schema resolution leaks internal URLs or enables SSRF | Security/privacy incident | Offline-only local catalogs and digest pinning; the v0.3 verifier has no network resolver. |
...
## 28. Locked v0.3 decisions and future considerations
...
7. **Release numbering:** `0.3` appears in predicates and schema paths; any validation-semantics correction uses a new immutable identifier rather than changing released bytes.

## 29. Definition of done

Makoto v0.3 is done only when:
...
- [ ] The website hosts immutable v0.3 schemas and a core walkthrough built from the same pinned demo artifacts.
...
```json
{
  ...
  "statements": [
    ...
    },
    {
      ...
      "predicateType": "https://usemakoto.dev/predicate/v0.3/origin",
...
- **Signature thresholds and signer lookup:** thresholds count distinct authorized signer IDs; v0.3 Ed25519 IDs are exact SPKI digests and keyless IDs are exact identity-constraint digests; duplicate, malformed, unknown, unsupported, and invalid extra signatures have defined behavior.
...
- **Legacy scope:** v0.3 does not invent an underspecified migration acceptance mode. Legacy bytes may be newly observed as ordinary artifacts, but historical v0.1 signer authenticity is never manufactured.
- **Negative-case precision:** edited metadata has an exact mutation recipe and primary stable error, while profile-substitution, distinct-signer threshold, multi-head, replay, evaluation-time, strict parser, and resource-limit fixtures were added to the test plan.
...
- **Give `deny` and `indeterminate` different process exit codes:** deferred as a preference. Machine consumers have the stable JSON decision; v0.3 retains a compact nonzero verification-failure code.
- **Add arbitrary CSV, Parquet, image, and binary semantic validators:** deferred. v0.3 hashes all finite byte artifacts but normatively validates JSON and finite NDJSON only.
- **Add global key discovery, public revocation, or open certificate federation:** deferred. v0.3 keyless verification uses only receiver-pinned identities and trusted-root bytes.
- **Treat newline, compression, or semantic-equivalent reserialization as one artifact:** rejected. v0.3 intentionally binds exact bytes; normalization is an attested transformation.
...
- **Network scope reduction:** automatic network schema retrieval was removed from v0.3. Hosted immutable schemas remain required distribution artifacts and are imported into authenticated offline catalogs before verification.
...
- **Failure-report completeness:** nullable unavailable values, unindexable envelopes, core-catalog digest, manifest/policy requirement flags, unknown-signer signatures, profile aggregation, exact summary counts, dataset/unreferenced-file records, deterministic primary error, and per-code context schemas are normative.
...
- **Emit an overall `indeterminate` decision from bundle verification:** rejected for v0.3. Required uncertainty denies; optional unavailable checks remain explicit `indeterminate` or `not_checked` records without changing an otherwise valid bundle from `allow`.
...
- **Remove exact-byte hashing, strict Ed25519, offline resolution, cycle defense, or artifact-only handoff requirements:** rejected; no reviewer supplied concrete harm that outweighed the locked v0.3 intent.
...
- **Profile and predicate interoperability:** profile closure is a load-time exact static `$ref` closure without unused supersets, duplicate anchors fail, v0.3 bundle decisions accept only core predicates, and all normative Unicode work uses vendored 15.0 tables.
...
- **Treat Windows ACL behavior as an untested reference-release MUST:** narrowed rather than accepted. Portable path rejection remains core; Windows runtime/ACL requirements apply only when an implementation claims Windows support, while v0.3 reference conformance remains macOS/Linux.
...
- **Handoff signature continuation:** a mixed-signature handoff whose remaining valid authorized signer IDs meet threshold continues through manifest authority, completeness, freshness, and profile checks while the strict signature failure independently denies. An unmet handoff threshold still skips authority-dependent work.
...
- **Catalog, requirements, and signatures:** declared catalog/resource absence is unambiguously `E_CATALOG_INVALID`; manifest profile requirements match the complete signed tuple; creation commands support deterministic repeated-key threshold signatures; `envelope cosign` accepts only the two v0.3 payload types.
...
- **Registry provenance:** the eight table byte identities are authoritative for v0.3, and Phase 0 must retain independent retrieval evidence rather than circularly comparing only vendored files with the same table.
...
| `E_PREDICATE_SEMANTICS_UNSUPPORTED` | 4 | A manifest-listed statement uses any non-core predicate type; v0.3 bundle decisions cannot install alternate graph semantics. |
...
| `E_SIGNATURE_INVALID` | 5 | A configured Ed25519 signature fails strict point encoding/decoding, subgroup, scalar, or equation verification, or a configured Sigstore signature fails bundle, chain, identity, transparency-log, trusted-time, or outer-envelope consistency verification after its transport fields pass Step 2. Transport failures remain `E_ENVELOPE_MALFORMED`. |
...
| `W_SIGNATURE_UNKNOWN` | 5 | A structurally valid signature names no configured Ed25519 key or keyless identity; it is not cryptographically evaluated or counted toward authorization. |
