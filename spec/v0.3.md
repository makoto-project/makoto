...
Makoto v0.3 makes two protocol changes:
...
Keyless and Sigstore signing remain out of scope. Index hierarchy, language metadata, sharding, scheduling, and AI-specific concepts remain extension or application concerns.
...
The checked-in end-to-end demonstration remains a v0.2 compatibility proof. v0.3 adds focused licence-profile and record-proof fixtures and tests; it does not relabel the v0.2 demo or claim a hosted v0.3 walkthrough.

This document carries the complete v0.2 protocol contract forward under a separate v0.3 identifier family and states the two v0.3 additions above. A release claim requires the reviewed prose, immutable schemas, verification-report schema, versioned `diagnostic-map.json`, applicable conformance vectors, and tagged checksum manifest. The current draft and candidate checksum inventory are not interoperability, hosting, or release evidence.
...
- DSSE remains algorithm-neutral, but the v0.3 trust-policy schema and reference implementation accept only Ed25519 verification keys. Verification MUST reject noncanonical `S >= L`, noncanonical point encodings, small-order public keys, and small-order `R` values, following the pinned strict vectors rather than permissive library defaults. A non-Ed25519 policy entry is invalid v0.3 consumer configuration and exits 2 before evidence evaluation; future signature algorithms require a versioned policy/schema extension.
...
Key revocation, certificate chains, identity federation, and transparency are out of scope for the v0.3 reference implementation. A local policy MAY remove a compromised key or constrain it with `validFrom` and `validUntil`. Those bounds are evaluated against the verifier’s current time or an explicit consumer-supplied evaluation time, never against the signer-asserted `event.occurredAt`. Backdating an event cannot restore authorization. Historical verification therefore requires an archived trust-policy snapshot or continued authorization of a retired public key; this is a conscious v0.3 operational limitation. A key can remain available for cryptographic verification while being unauthorized by the active policy.
...
- **Signature thresholds and key lookup:** thresholds count distinct authorized keys; v0.3 key IDs are exact SPKI digests; duplicate, malformed, unknown, unsupported, and invalid extra signatures have defined behavior.
