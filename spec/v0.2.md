...
- Isolated builders, hardware-backed keys, remote attestation, or control-plane-generated evidence. Section 8.3 reserves a future level above L3 for them; no current level claims them.
...
Makoto keeps SLSA-style progressive levels. A level is a named, cumulative set of the verification-report checks in Section 15, so a receiver can state a requirement such as “L2 or higher” and still read exactly which check fell short. Levels are derived only from a completed report that validates against the core `verification-report` schema. Deriving a level MUST NOT change any check status, diagnostic, exit code, or the `allow`/`deny` decision, and a level is always relative to the receiver policy, catalog, and expectations that produced the report.

| Level | Name | Every listed top-level check is `pass` | Also permitted |
| --- | --- | --- | --- |
| L1 | Provenance is authentic | `load-safely`, `parse-strictly`, `index-payloads`, `core-schemas`, `signatures` | — |
| L2 | Provenance is authorized and complete | L1, plus `authorization-thresholds`, `authorization`, `graph`, `roots-and-heads`, `completeness-anchor`, `artifact-bytes` | `metadata-profiles`, `graph-dependency-artifacts`, and `artifact-profiles` MAY be `not_checked` instead of `pass` |
| L3 | Provenance is anchored | L2, plus `freshness-anchors` | The report `decision` MUST be `allow` |

- Levels are cumulative. A report that does not satisfy L1 has no level, even if later checks passed.
- `not_checked` is accepted only where the table allows it, because Section 15.1 limits that status to optional, waived, or empty populations. `skipped`, `fail`, and `indeterminate` never satisfy a level.
- L3 requires `freshness-anchors: pass`. An `allowReplayableHandoff` policy that records freshness as `not_checked` can yield `allow` but stops at L2.
- L1 means the exact statement bytes are well-formed and signed by configured keys. It does not mean those keys were authorized, that the graph is complete, or that the received artifact bytes match.
- L2 adds receiver authorization of every signer, a continuous graph from each head to its origins, an authorized handoff whose exact sets match, and received final-artifact bytes that match the signed digests.
- L3 adds an independent freshness anchor. It is the level a receiver needs before treating a handoff as current.
- No level asserts that a signer told the truth, that a transformation executed as described, or that an authorized key holder could not have produced false evidence (Section 8.2). A future level for isolated evidence generation is reserved and undefined until its evidence, schemas, and conformance vectors exist.

The reference CLI derives levels with `makoto report level` (Section 16.1). With `--json` it writes one deterministic JSON object with exactly `decision`, `level` (`"L1"`, `"L2"`, `"L3"`, or `null`), and `levels`, an array in L1, L2, L3 order of objects with exactly `decisionRequired`, `level`, `satisfied`, `title`, and `unmetChecks` (an array of `{ "id", "status" }` in canonical check order). When `--require <level>` is supplied the object also contains `required` and `meetsRequired`. The command exits 0 when no level is required or the achieved level meets the requirement, 1 when it does not, and 2 when the input is not a schema-valid verification report.
...
makoto report level <verification-report.json> [--require L1|L2|L3] [--json]
...
L1, L2, and L3 are the current Makoto assurance levels, defined in Section 8.3: L1 authentic, L2 authorized and complete, L3 anchored. Documentation for L1, L2, and L3 MUST describe those Section 8.3 definitions. Only the v0.1 ladder semantics are historical: an L2 bound to Sigstore identity and trusted timestamps, and an L3 claiming platform-generated evidence that no key holder could forge. Those requirements MUST NOT be presented as current, and a v0.1 level claim MUST NOT be reported as a v0.2 level.
